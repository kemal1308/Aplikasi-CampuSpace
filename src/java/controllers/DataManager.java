...
public class DataManager {
    ...
    private String getAutoGambarUrl(String urlDariDB, String tipe, String namaFasilitas) {
        if (urlDariDB != null && !urlDariDB.trim().isEmpty()) return urlDariDB; 
        String nama = namaFasilitas.toLowerCase();
        if ("RUANGAN".equals(tipe)) {
            if (nama.contains("auditorium") || nama.contains("aula") || nama.contains("damar") || nama.contains("teater")) return "https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=800&q=80"; 
            else if (nama.contains("lapangan") || nama.contains("sport") || nama.contains("futsal") || nama.contains("basket")) return "https://images.unsplash.com/photo-1526232761682-d26e03ac148e?w=800&q=80";
            else if (nama.contains("lab") || nama.contains("laboratorium") || nama.contains("komputer")) return "https://images.unsplash.com/photo-1517048676732-d65bc937f952?w=800&q=80";
            else return "https://images.unsplash.com/photo-1580582932707-520aed937b7b?w=800&q=80"; 
        } else {
            if (nama.contains("proyektor") || nama.contains("projector")) return "https://images.unsplash.com/photo-1574717024453-354056aafd6c?w=800&q=80";
            else if (nama.contains("mic") || nama.contains("suara") || nama.contains("speaker") || nama.contains("shure")) return "https://images.unsplash.com/photo-1590602847861-f357a9332bbc?w=800&q=80";
            else if (nama.contains("kamera") || nama.contains("camera") || nama.contains("dslr")) return "https://images.unsplash.com/photo-1516035069371-29a1b244cc32?w=800&q=80";
            else return "https://images.unsplash.com/photo-1527443224154-c4a3942d3acf?w=800&q=80"; 
        }
    }
    ...
    public boolean simpanReservasi(String idPeminjaman, String idFasilitas, String idPengguna, String tglPinjam, String tglKembali, String keperluan, String jamMulai, String jamSelesai) {
        String query = "INSERT INTO peminjaman (id_peminjaman, id_fasilitas, id_pengguna, tanggal_pinjam, tanggal_kembali, status_peminjaman, total_denda, status_denda, keperluan, jam_mulai, jam_selesai) VALUES (?, ?, ?, CAST(? AS date), CAST(? AS date), 'PENDING', 0, 'BELUM_DIBAYAR', ?, CAST(? AS time), CAST(? AS time))";
        try (java.sql.Connection conn = DatabaseConnection.getConnection();
             java.sql.PreparedStatement stmtInsert = conn.prepareStatement(query)) {
            stmtInsert.setString(1, idPeminjaman);
            stmtInsert.setString(2, idFasilitas);
            stmtInsert.setString(3, idPengguna);
            stmtInsert.setString(4, tglPinjam);
            stmtInsert.setString(5, tglKembali);
            stmtInsert.setString(6, keperluan);
            stmtInsert.setString(7, jamMulai);
            stmtInsert.setString(8, jamSelesai);
            return stmtInsert.executeUpdate() > 0;
        } catch (java.sql.SQLException e) { e.printStackTrace(); return false; }
    }

    // CEK BENTROK: Tanggal dicek irisannya, DAN Jam dicek irisannya (Konsep Slot Harian - Menerima 5 Parameter)
    public String cekKetersediaanJadwal(String idFasilitas, String tglMulaiBaru, String tglSelesaiBaru, String jamMulaiBaru, String jamSelesaiBaru) {
        String query = "SELECT tanggal_pinjam, tanggal_kembali, jam_mulai, jam_selesai FROM peminjaman " +
                       "WHERE id_fasilitas = ? AND status_peminjaman IN ('PENDING', 'APPROVED', 'DIPINJAM') " +
                       "AND (CAST(? AS date) <= tanggal_kembali AND CAST(? AS date) >= tanggal_pinjam) " +
                       "AND (CAST(? AS time) < COALESCE(jam_selesai, '21:00:00'::time) AND CAST(? AS time) > COALESCE(jam_mulai, '07:00:00'::time))";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, idFasilitas);
            stmt.setString(2, tglMulaiBaru);
            stmt.setString(3, tglSelesaiBaru);
            stmt.setString(4, jamMulaiBaru);
            stmt.setString(5, jamSelesaiBaru);
            try (ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) {
                    String jMulai = rs.getTime("jam_mulai") != null ? rs.getTime("jam_mulai").toString().substring(0,5) : "07:00";
                    String jSelesai = rs.getTime("jam_selesai") != null ? rs.getTime("jam_selesai").toString().substring(0,5) : "21:00";
                    String dateMulai = rs.getDate("tanggal_pinjam").toString();
                    String dateSelesai = rs.getDate("tanggal_kembali").toString();
                    
                    if (dateMulai.equals(dateSelesai)) {
                        return dateMulai + " (" + jMulai + "-" + jSelesai + " WIB)";
                    } else {
                        return dateMulai + " s/d " + dateSelesai + " (" + jMulai + "-" + jSelesai + " WIB)";
                    }
                }
            }
        } catch (SQLException e) { e.printStackTrace(); }
    ...
    public List<String> getJadwalTerbooking(String idFasilitas) {
        ...
        String query = "SELECT tanggal_pinjam, tanggal_kembali, jam_mulai, jam_selesai FROM peminjaman WHERE id_fasilitas = ? AND status_peminjaman IN ('PENDING', 'APPROVED', 'DIPINJAM') AND tanggal_kembali >= CURRENT_DATE ORDER BY tanggal_pinjam ASC";
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, idFasilitas);
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    String jMulai = rs.getTime("jam_mulai") != null ? rs.getTime("jam_mulai").toString().substring(0,5) : "07:00";
                    String jSelesai = rs.getTime("jam_selesai") != null ? rs.getTime("jam_selesai").toString().substring(0,5) : "21:00";
                    listJadwal.add("• " + rs.getDate("tanggal_pinjam").toString() + " (" + jMulai + " - " + jSelesai + " WIB) s/d " + rs.getDate("tanggal_kembali").toString());
                }
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return listJadwal;
    ...
    public java.util.Map<String, java.util.List<String>> getAllJadwalAktif() {
        ...
        String query = "SELECT id_fasilitas, tanggal_pinjam, tanggal_kembali, jam_mulai, jam_selesai FROM peminjaman WHERE status_peminjaman IN ('PENDING', 'APPROVED', 'DIPINJAM') AND tanggal_kembali >= CURRENT_DATE ORDER BY tanggal_pinjam ASC";
        try (java.sql.Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                String jMulai = rs.getTime("jam_mulai") != null ? rs.getTime("jam_mulai").toString().substring(0,5) : "07:00";
                String jSelesai = rs.getTime("jam_selesai") != null ? rs.getTime("jam_selesai").toString().substring(0,5) : "21:00";
                String infoJadwal = "• " + rs.getDate("tanggal_pinjam").toString() + " (" + jMulai + " - " + jSelesai + " WIB) s/d " + rs.getDate("tanggal_kembali").toString();
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    p.setJamMulai(rs.getTime("jam_mulai")); 
                    p.setJamSelesai(rs.getTime("jam_selesai")); 
    ...
    public String[] prosesLoginSSO(String emailInput, String roleInput) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            try (ResultSet rs = stmtCheck.executeQuery()) {
                if (rs.next()) { return new String[] {rs.getString("id_pengguna"), rs.getString("nama_pengguna"), rs.getString("tipe_pengguna")}; } 
                else {
                    ...
                    try (PreparedStatement stmtInsert = conn.prepareStatement(queryInsert)) {
                        ...
                        stmtInsert.setInt(4, tipePengguna.equals("DOSEN") ? 7 : 3);
                        stmtInsert.setInt(5, tipePengguna.equals("DOSEN") ? 5 : 2);
    ...
    public String getNamaFasilitas(String idFasilitas) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, idFasilitas);
            try (ResultSet rs = stmt.executeQuery()) { if (rs.next()) { nama = rs.getString("nama_fasilitas"); } }
        } catch (SQLException e) { e.printStackTrace(); }
        return nama;
    }
    ...
    public int getMaxPinjam(String idPengguna) {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
             PreparedStatement stmt = conn.prepareStatement(query)) {
            stmt.setString(1, idPengguna);
            try (ResultSet rs = stmt.executeQuery()) { if (rs.next()) { maxHari = rs.getInt("max_pinjam"); } }
        } catch (SQLException e) { e.printStackTrace(); }
        return maxHari;
    }
    ...
    public List<Peminjaman> getPendingReservasi() {
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                } catch (Exception e) { p.setKeperluan("-"); }                
                list.add(p);
            }
        } catch (SQLException e) { e.printStackTrace(); }
        return list;
    ...
    public void cekDanUpdateDendaOtomatis() {
        String querySelect = "SELECT p.id_peminjaman, p.tanggal_kembali, f.tipe_fasilitas " +
                             "FROM peminjaman p " +
                             "JOIN fasilitas f ON p.id_fasilitas = f.id_fasilitas " +
                             "WHERE p.status_peminjaman = 'APPROVED' AND p.tanggal_kembali < CURRENT_DATE";
        ...
        try (java.sql.Connection conn = DatabaseConnection.getConnection();
            ...
            while (rs.next()) {
                ...
                String tipeFasilitas = rs.getString("tipe_fasilitas"); 
                ...
                if (selisihHari > 0) {
                    ...
                    if ("RUANGAN".equals(tipeFasilitas)) {
                        dendaPerHari = 50000;
                    } else if ("ALAT_ELEKTRONIK".equals(tipeFasilitas)) {
                        dendaPerHari = 20000;
                    } else {
                        dendaPerHari = 20000;
                    }
                    
                    long totalDenda = selisihHari * dendaPerHari;
    ...
    public int getJumlahReservasiSelesaiTerfilter(int bulan, int tahun) {
        ...
        try (java.sql.Connection conn = DatabaseConnection.getConnection();
            ...
            try (java.sql.ResultSet rs = stmt.executeQuery()) {
                if (rs.next()) { jumlah = rs.getInt("total"); }
            }
        } catch (java.sql.SQLException e) { e.printStackTrace(); }
        return jumlah;
    }
